...
class PenyakitParu {
  ...
  static final Map<String, Map<String, String>> penyakitInfo = {
    'Risiko Tinggi': { //Persentase lebih dari 80%
      'description': 'Anda memiliki faktor risiko utama yang signifikan terhadap kanker paru-paru. Diperlukan pemeriksaan lanjutan seperti CT-Scan Dada dan konsultasi segera ke spesialis paru.',
      'treatment': 'Segera konsultasi ke dokter spesialis paru, lakukan pemeriksaan imaging, dan pertimbangkan tindakan deteksi dini.',
      'icon': 'exclamationmark_shield_fill',
    },
    'Risiko Sedang': { //Persentase lebih dari 50% dan kurang dari 80%
      'description': 'Anda memiliki beberapa faktor risiko untuk kanker paru-paru. Pemantauan rutin dan pemeriksaan lebih lanjut disarankan.',
      'treatment': 'Lakukan pemeriksaan kesehatan berkala dan konsultasi dengan dokter untuk evaluasi lanjutan.',
      'icon': 'exclamationmark_shield',
    },
    'Risiko Rendah': { //Persentase lebih dari 20% dan kurang dari 50%
      'description': 'Faktor risiko Anda terhadap kanker paru-paru terdeteksi minimal. Tetap jaga gaya hidup sehat dan lakukan pemeriksaan rutin.',
      'treatment': 'Pertahankan pola hidup sehat, hindari paparan risiko, dan lakukan pemeriksaan kesehatan secara rutin.',
      'icon': 'waveform_path',
    },
    'Tidak Ada Risiko': { //Persentase kurang dari 20%
      'description': 'Tidak ditemukan faktor risiko kanker paru-paru berdasarkan jawaban Anda. Tetap lakukan pencegahan dan pemeriksaan rutin untuk menjaga kesehatan.',
      'treatment': 'Teruskan gaya hidup sehat, hindari rokok dan polusi, serta lakukan skrining kesehatan secara berkala.',
...
class _OutputPageState extends State<OutputPage> with SingleTickerProviderStateMixin {
  ...
  Future<File> _generatePDF() async {
        ...
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              pw.Center(
                child: pw.Text(
                  'HASIL DIAGNOSA RISIKO KANKER PARU',
                  style: pw.TextStyle(
                    fontSize: 18, 
    ...
    final file = File('${output.path}/hasil_analisis_risiko_kanker_paru.pdf');
  ...
  Future<void> _shareResults() async {
    try {
      ...
      await Share.shareXFiles(
        [XFile(pdfFile.path)],
        text: 'Hasil Analisis Risiko Kanker Paru',
        subject: 'Analisis Risiko Kanker Paru - ${DateFormat('dd MMM yyyy').format(DateTime.now())}',
      );
    } catch (e) {
      if (!mounted) return;
