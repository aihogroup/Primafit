...
import 'package:primafit/features/cancer_screening/presentation/prostat/output_prostat.dart';
import 'package:primafit/features/cancer_screening/presentation/prostat/list_pertanyaan.dart';

class KankerProstatPage extends StatefulWidget {
  const KankerProstatPage({super.key});

  @override
  _KankerProstatPageState createState() => _KankerProstatPageState();
}

class _KankerProstatPageState extends State<KankerProstatPage> with SingleTickerProviderStateMixin {
  ...
  final bool _loading = false; // Set to false to avoid unnecessary loading screen
  ...
  void _nextPage() {
    if (_currentPage < KankerProstatQuestions.activeQuestions(_answers).length - 1) {
      _animateToNextPage();
    } else {
      _submitAnswers();
    }
  ...
  Future<void> _submitAnswers() async {
    ...
    try {
      ...
      for (var question in KankerProstatQuestions.questions) {
        final String id = question['id'];
        
        // Check if this question should be skipped
        bool shouldSkip = false;
        for (var otherQuestion in KankerProstatQuestions.questions) {
      ...
      await DatabaseHelperProstat.instance.saveUserAnswers(userAnswers);
      ...
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(builder: (context) => const OutputPageProstat()),
      );
    } catch (e) {
      if (!mounted) return;
          ...
          onTap: () {
            setState(() {
              ...
              if (answer == 'Ya') {
                ...
                final currentQuestion = KankerProstatQuestions.questions.firstWhere(
                  (q) => q['id'] == questionId,
                  orElse: () => {},
                );
                ...
                if (currentQuestion.containsKey('skipRelatedQuestions')) {
                  final List<String> skipQuestions = List<String>.from(currentQuestion['skipRelatedQuestions']);
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Text(
                      'Pertanyaan ${index + 1}/${KankerProstatQuestions.activeQuestions(_answers).length}',
                      style: GoogleFonts.poppins(
                        fontSize: 14,
                        fontWeight: FontWeight.w500,
                        color: Colors.grey.shade700,
                      ...
                      child: Stack(
                        children: [
                          AnimatedFractionallySizedBox(
                            duration: const Duration(milliseconds: 300),
                            widthFactor: (index + 1) / KankerProstatQuestions.activeQuestions(_answers).length,
                            child: Container(
                              decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(10),
                ...
                Row(
                  ...
                  children: [
                    ...
                    if (index < KankerProstatQuestions.activeQuestions(_answers).length - 1)
                      ElevatedButton.icon(
  ...
  Widget build(BuildContext context) {
    ...
    final activeQuestions = KankerProstatQuestions.activeQuestions(_answers);
