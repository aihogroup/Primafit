...
import 'package:primafit/features/cancer_screening/data/database_rahim.dart';
...
class PenyakitRahim {
  ...
  static final Map<String, Map<String, String>> penyakitInfo = {
    'Risiko Tinggi': { //Persentase lebih dari 80%
      'description': 'Anda memiliki faktor risiko yang signifikan untuk kanker rahim (endometrium). Diperlukan pemeriksaan lanjutan seperti USG transvaginal dan konsultasi ke dokter spesialis ginekologi segera.',
      'treatment': 'Segera lakukan pemeriksaan USG transvaginal, biopsi endometrium jika diperlukan, dan konsultasi dengan dokter spesialis ginekologi untuk evaluasi menyeluruh.',
      'icon': 'exclamationmark_shield_fill',
    },
    'Risiko Sedang': { //Persentase lebih dari 50% dan kurang dari 80%
      'description': 'Anda memiliki beberapa faktor risiko untuk kanker rahim. Disarankan untuk melakukan pemantauan rutin dan pemeriksaan lebih lanjut.',
      'treatment': 'Lakukan pemeriksaan USG pelvis secara rutin, konsultasi dengan dokter ginekologi, dan pertimbangkan USG transvaginal tahunan untuk deteksi dini.',
      'icon': 'exclamationmark_shield',
    },
    'Risiko Rendah': { //Persentase lebih dari 20% dan kurang dari 50%
      'description': 'Faktor risiko Anda terhadap kanker rahim terdeteksi minimal. Tetap jaga gaya hidup sehat dan lakukan pemeriksaan rutin.',
      'treatment': 'Pertahankan berat badan ideal, rutin kunjungan ke dokter ginekologi untuk pemeriksaan panggul tahunan, dan laporkan segera jika terjadi perdarahan abnormal.',
      'icon': 'waveform_path',
    },
    'Tidak Ada Risiko': { //Persentase kurang dari 20%
      'description': 'Tidak ditemukan faktor risiko signifikan kanker rahim berdasarkan jawaban Anda. Tetap lakukan pemeriksaan rutin untuk pencegahan.',
      'treatment': 'Tetap jaga pola makan seimbang, pertahankan berat badan ideal, rutin berolahraga, dan tetap lakukan pemeriksaan ginekologi secara rutin.',
      'icon': 'checkmark_shield_fill',
    },
  };
}

class OutputPageRahim extends StatefulWidget {
  const OutputPageRahim({super.key});

  @override
  _OutputPageRahimState createState() => _OutputPageRahimState();
}

class _OutputPageRahimState extends State<OutputPageRahim> with SingleTickerProviderStateMixin {
  ...
  Future<File> _generatePDF() async {
        ...
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              pw.Center(
                child: pw.Text(
                  'HASIL DIAGNOSA RISIKO KANKER RAHIM',
                  style: pw.TextStyle(
                    fontSize: 18, 
              ...
              pw.SizedBox(height: 20),
              if (_diagnosisResults.isNotEmpty && PenyakitRahim.penyakitInfo.containsKey(_diagnosisResults[0]['penyakit']))
                pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text(
                    ...
                    pw.SizedBox(height: 5),
                    pw.Text(
                      PenyakitRahim.penyakitInfo[_diagnosisResults[0]['penyakit']]!['description'] ?? '',
                    ),
                    ...
                    pw.SizedBox(height: 5),
                    pw.Text(
                      PenyakitRahim.penyakitInfo[_diagnosisResults[0]['penyakit']]!['treatment'] ?? '',
                    ),
                  ],
                ),
              pw.SizedBox(height: 20),
              ...
              pw.SizedBox(height: 5),
              pw.Text(
                'Hasil diagnosa ini hanya sebagai rujukan awal berdasarkan faktor risiko yang diidentifikasi. Konsultasikan dengan dokter atau tenaga medis profesional untuk diagnosis dan pengobatan yang tepat. Deteksi dini kanker rahim melalui pemeriksaan ginekologi rutin sangat penting untuk pengobatan yang efektif.',
                style: pw.TextStyle(
    ...
    final file = File('${output.path}/hasil_analisis_risiko_kanker_rahim.pdf');
  ...
  Future<void> _shareResults() async {
    try {
      ...
      await Share.shareXFiles(
        [XFile(pdfFile.path)],
        text: 'Hasil Analisis Risiko Kanker Rahim',
        subject: 'Analisis Risiko Kanker Rahim - ${DateFormat('dd MMM yyyy').format(DateTime.now())}',
      );
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _loadDiagnosisResults() async {
    try {
      final results = await DatabaseHelperRahim.instance.getDiagnosisResult(_threshold);
  ...
  Color _getRiskColor(String riskCategory) {
    switch (riskCategory) {
      ...
      case 'Tidak Ada Risiko':
        return Colors.green;
      default:
        return const Color(0xFF64D1DE); // Pink color for uterine cancer theme
    }
  }
  ...
  Widget _buildResultCard(Map<String, dynamic> result) {
    ...
    final Map<String, String>? info = PenyakitRahim.penyakitInfo[penyakit];
