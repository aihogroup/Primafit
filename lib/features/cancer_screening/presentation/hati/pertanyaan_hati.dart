...
import 'package:primafit/features/cancer_screening/presentation/hati/output_hati.dart';
import 'package:primafit/features/cancer_screening/presentation/hati/list_pertanyaan.dart';

class KankerHatiPage extends StatefulWidget {
  const KankerHatiPage({super.key});

  @override
  _KankerHatiPageState createState() => _KankerHatiPageState();
}

class _KankerHatiPageState extends State<KankerHatiPage> with SingleTickerProviderStateMixin {
  ...
  void _nextPage() {
    if (_currentPage < KankerHatiQuestions.activeQuestions(_answers).length - 1) {
      _animateToNextPage();
    } else {
      _submitAnswers();
    }
  ...
  Future<void> _submitAnswers() async {
    ...
    try {
      ...
      for (var question in KankerHatiQuestions.questions) {
        final String id = question['id'];
        
        // Check if this question should be skipped
        bool shouldSkip = false;
        for (var otherQuestion in KankerHatiQuestions.questions) {
      ...
      await DatabaseHelperHati.instance.saveUserAnswers(userAnswers);
      ...
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(builder: (context) => const OutputPageHati()),
      );
    } catch (e) {
      if (!mounted) return;
          ...
          onTap: () {
            setState(() {
              ...
              if (answer == 'Ya') {
                ...
                final currentQuestion = KankerHatiQuestions.questions.firstWhere(
                  (q) => q['id'] == questionId,
                  orElse: () => {},
                );
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Text(
                      'Pertanyaan ${index + 1}/${KankerHatiQuestions.activeQuestions(_answers).length}',
                      style: GoogleFonts.poppins(
                        fontSize: 14,
                        fontWeight: FontWeight.w500,
                        color: Colors.grey.shade700,
                      ...
                      child: Stack(
                        children: [
                          AnimatedFractionallySizedBox(
                            duration: const Duration(milliseconds: 300),
                            widthFactor: (index + 1) / KankerHatiQuestions.activeQuestions(_answers).length,
                            child: Container(
                              decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(10),
                ...
                Row(
                  ...
                  children: [
                    ...
                    if (index < KankerHatiQuestions.activeQuestions(_answers).length - 1)
                      ElevatedButton.icon(
  ...
  Widget build(BuildContext context) {
    ...
    final activeQuestions = KankerHatiQuestions.activeQuestions(_answers);
