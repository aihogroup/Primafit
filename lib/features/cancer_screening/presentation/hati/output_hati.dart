...
import 'package:primafit/features/cancer_screening/data/database_hati.dart';
...
class PenyakitHati {
  ...
  static final Map<String, Map<String, String>> penyakitInfo = {
    'Risiko Tinggi': { //Persentase lebih dari 80%
      'description': 'Anda memiliki faktor risiko yang signifikan untuk kanker hati. Diperlukan pemeriksaan lanjutan seperti USG hati, CT scan atau MRI, dan konsultasi ke dokter spesialis hepatologi segera.',
      'treatment': 'Segera lakukan pemeriksaan pencitraan (USG/CT scan/MRI), tes fungsi hati, dan konsultasi dengan dokter spesialis. Perlu pertimbangan biopsi untuk konfirmasi diagnosis.',
      'icon': 'exclamationmark_shield_fill',
    },
    'Risiko Sedang': { //Persentase lebih dari 50% dan kurang dari 80%
      'description': 'Anda memiliki beberapa faktor risiko untuk kanker hati. Disarankan untuk melakukan pemantauan rutin dan pemeriksaan lebih lanjut.',
      'treatment': 'Lakukan pemeriksaan USG hati setiap 6 bulan, tes fungsi hati secara berkala, dan atasi faktor risiko yang dapat dimodifikasi seperti hepatitis, diabetes, atau konsumsi alkohol.',
      'icon': 'exclamationmark_shield',
    },
    'Risiko Rendah': { //Persentase lebih dari 20% dan kurang dari 50%
      'description': 'Faktor risiko Anda terhadap kanker hati terdeteksi minimal. Tetap jaga gaya hidup sehat dan lakukan pemeriksaan rutin.',
      'treatment': 'Pertahankan berat badan ideal, batasi konsumsi alkohol, dan lakukan pemeriksaan kesehatan rutin termasuk tes fungsi hati setahun sekali.',
      'icon': 'waveform_path',
    },
    'Tidak Ada Risiko': { //Persentase kurang dari 20%
      'description': 'Tidak ditemukan faktor risiko signifikan kanker hati berdasarkan jawaban Anda. Tetap lakukan pemeriksaan rutin untuk pencegahan.',
      'treatment': 'Pertahankan gaya hidup sehat dengan diet seimbang, olahraga teratur, hindari konsumsi alkohol berlebih, dan vaksinasi hepatitis B jika belum.',
      'icon': 'checkmark_shield_fill',
    },
  };
}

class OutputPageHati extends StatefulWidget {
  const OutputPageHati({super.key});

  @override
  _OutputPageHatiState createState() => _OutputPageHatiState();
}

class _OutputPageHatiState extends State<OutputPageHati> with SingleTickerProviderStateMixin {
  ...
  Future<File> _generatePDF() async {
        ...
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              pw.Center(
                child: pw.Text(
                  'HASIL DIAGNOSA RISIKO KANKER HATI',
                  style: pw.TextStyle(
                    fontSize: 18, 
              ...
              pw.SizedBox(height: 20),
              if (_diagnosisResults.isNotEmpty && PenyakitHati.penyakitInfo.containsKey(_diagnosisResults[0]['penyakit']))
                pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text(
                    ...
                    pw.SizedBox(height: 5),
                    pw.Text(
                      PenyakitHati.penyakitInfo[_diagnosisResults[0]['penyakit']]!['description'] ?? '',
                    ),
                    ...
                    pw.SizedBox(height: 5),
                    pw.Text(
                      PenyakitHati.penyakitInfo[_diagnosisResults[0]['penyakit']]!['treatment'] ?? '',
                    ),
                  ],
                ),
              pw.SizedBox(height: 20),
              ...
              pw.SizedBox(height: 5),
              pw.Text(
                'Hasil diagnosa ini hanya sebagai rujukan awal berdasarkan faktor risiko yang diidentifikasi. Konsultasikan dengan dokter atau tenaga medis profesional untuk diagnosis dan pengobatan yang tepat. Deteksi dini kanker hati melalui USG, CT scan, atau MRI dan tes darah sangat penting untuk pengobatan yang efektif.',
                style: pw.TextStyle(
    ...
    final file = File('${output.path}/hasil_analisis_risiko_kanker_hati.pdf');
  ...
  Future<void> _shareResults() async {
    try {
      ...
      await Share.shareXFiles(
        [XFile(pdfFile.path)],
        text: 'Hasil Analisis Risiko Kanker Hati',
        subject: 'Analisis Risiko Kanker Hati - ${DateFormat('dd MMM yyyy').format(DateTime.now())}',
      );
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _loadDiagnosisResults() async {
    try {
      final results = await DatabaseHelperHati.instance.getDiagnosisResult(_threshold);
  ...
  Color _getRiskColor(String riskCategory) {
    switch (riskCategory) {
      ...
      case 'Tidak Ada Risiko':
        return Colors.green;
      default:
        return const Color(0xFF64D1DE); // Green color for liver cancer theme
    }
  }
  ...
  Widget _buildResultCard(Map<String, dynamic> result) {
    ...
    final Map<String, String>? info = PenyakitHati.penyakitInfo[penyakit];
