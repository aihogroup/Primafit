...
class DatabaseParenting {
  ...
  Future<List<Map<String, dynamic>>> getVaccinationSchedules({int? ageMonths}) async {
    Database db = await database;
    String whereClause = ageMonths != null ? 'age_months <= ?' : '';
    List<dynamic> whereArgs = ageMonths != null ? [ageMonths] : [];
  ...
  Future<List<Map<String, dynamic>>> getHealthRecords(int childId, {String? recordType}) async {
    ...
    List<dynamic> whereArgs = [childId];
  ...
  Future<Map<String, dynamic>> getChildStatistics(int childId) async {
    ...
    var child = await getChildById(childId);
    if (child == null) return {};
    
    DateTime birthDate = DateTime.parse(child['date_of_birth']);
    int ageInMonths = DateTime.now().difference(birthDate).inDays ~/ 30;
    
    // Get milestone progress
    var milestoneProgress = await db.rawQuery('''
      SELECT 
    ...
    var latestGrowth = await db.query(
      'growth_records',
      where: 'child_id = ?',
      whereArgs: [childId],
      orderBy: 'record_date DESC',
    ...
    var vaccinationProgress = await db.rawQuery('''
      SELECT 
    ...
    // Get total photos
    var photoCount = await db.rawQuery('''
  ...
  Future<List<Map<String, dynamic>>> getUpcomingReminders({int days = 7}) async {
    Database db = await database;
    String endDate = DateTime.now().add(Duration(days: days)).toIso8601String().split('T')[0];
  ...
  Future<bool> importData(String jsonData) async {
    try {
      ...
      Map<String, dynamic> importData = {}; // Parse jsonData
      ...
      for (String tableName in importData.keys) {
        if (tableName != 'export_date' && tableName != 'version') {
          List<dynamic> tableData = importData[tableName];
  ...
  Future<void> _clearAllTables(Database db) async {
    List<String> tableNames = [
      'children', 'child_milestones', 'growth_records', 'child_vaccinations',
      'health_records', 'photo_memories', 'sleep_records', 'nutrition_records',
      'reminders', 'emergency_contacts'
    ];
