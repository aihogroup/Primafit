...
import 'package:primafit/features/cancer_screening/data/database_prostat.dart';
import 'package:share_plus/share_plus.dart';
import 'package:pdf/widgets.dart' as pw;
import 'package:path_provider/path_provider.dart';
import 'dart:io';

class PenyakitProstat {
  ...
  static final Map<String, Map<String, String>> penyakitInfo = {
    'Risiko Tinggi': { //Persentase lebih dari 80%
      'description': 'Anda memiliki faktor risiko yang signifikan untuk kanker prostat. Diperlukan pemeriksaan lanjutan seperti PSA (Prostate-Specific Antigen), DRE (Digital Rectal Examination), dan konsultasi ke dokter spesialis urologi segera.',
      'treatment': 'Segera lakukan pemeriksaan PSA, DRE, dan konsultasi dengan dokter spesialis urologi untuk evaluasi menyeluruh. Biopsi prostat mungkin diperlukan untuk konfirmasi diagnosis.',
      'icon': 'exclamationmark_shield_fill',
    },
    'Risiko Sedang': { //Persentase lebih dari 50% dan kurang dari 80%
      'description': 'Anda memiliki beberapa faktor risiko untuk kanker prostat. Disarankan untuk melakukan pemantauan rutin dan pemeriksaan lebih lanjut.',
      'treatment': 'Lakukan pemeriksaan PSA secara rutin, konsultasi dengan dokter urologi, dan pertimbangkan pemeriksaan tahunan untuk deteksi dini.',
      'icon': 'exclamationmark_shield',
    },
    'Risiko Rendah': { //Persentase lebih dari 20% dan kurang dari 50%
      'description': 'Faktor risiko Anda terhadap kanker prostat terdeteksi minimal. Tetap jaga gaya hidup sehat dan lakukan pemeriksaan rutin.',
      'treatment': 'Pertahankan berat badan ideal, konsumsi makanan sehat, rutin berolahraga, dan lakukan kunjungan ke dokter untuk pemeriksaan secara berkala.',
      'icon': 'waveform_path',
    },
    'Tidak Ada Risiko': { //Persentase kurang dari 20%
      'description': 'Tidak ditemukan faktor risiko signifikan kanker prostat berdasarkan jawaban Anda. Tetap lakukan pemeriksaan rutin untuk pencegahan.',
      'treatment': 'Tetap jaga pola makan seimbang, pertahankan berat badan ideal, rutin berolahraga, dan tetap lakukan pemeriksaan kesehatan secara rutin.',
      'icon': 'checkmark_shield_fill',
    },
  };
}

class OutputPageProstat extends StatefulWidget {
  const OutputPageProstat({super.key});

  @override
  _OutputPageProstatState createState() => _OutputPageProstatState();
}

class _OutputPageProstatState extends State<OutputPageProstat> with SingleTickerProviderStateMixin {
  ...
  Future<File> _generatePDF() async {
        ...
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              pw.Center(
                child: pw.Text(
                  'HASIL DIAGNOSA RISIKO KANKER PROSTAT',
                  style: pw.TextStyle(
                    fontSize: 18, 
              ...
              pw.SizedBox(height: 20),
              if (_diagnosisResults.isNotEmpty && PenyakitProstat.penyakitInfo.containsKey(_diagnosisResults[0]['penyakit']))
                pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text(
                    ...
                    pw.SizedBox(height: 5),
                    pw.Text(
                      PenyakitProstat.penyakitInfo[_diagnosisResults[0]['penyakit']]!['description'] ?? '',
                    ),
                    ...
                    pw.SizedBox(height: 5),
                    pw.Text(
                      PenyakitProstat.penyakitInfo[_diagnosisResults[0]['penyakit']]!['treatment'] ?? '',
                    ),
                  ],
                ),
              pw.SizedBox(height: 20),
              ...
              pw.SizedBox(height: 5),
              pw.Text(
                'Hasil diagnosa ini hanya sebagai rujukan awal berdasarkan faktor risiko yang diidentifikasi. Konsultasikan dengan dokter atau tenaga medis profesional untuk diagnosis dan pengobatan yang tepat. Deteksi dini kanker prostat melalui pemeriksaan PSA dan pemeriksaan urologi rutin sangat penting untuk pengobatan yang efektif.',
                style: pw.TextStyle(
    ...
    final file = File('${output.path}/hasil_analisis_risiko_kanker_prostat.pdf');
  ...
  Future<void> _shareResults() async {
    try {
      ...
      await Share.shareXFiles(
        [XFile(pdfFile.path)],
        text: 'Hasil Analisis Risiko Kanker Prostat',
        subject: 'Analisis Risiko Kanker Prostat - ${DateFormat('dd MMM yyyy').format(DateTime.now())}',
      );
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _loadDiagnosisResults() async {
    try {
      final results = await DatabaseHelperProstat.instance.getDiagnosisResult(_threshold);
  ...
  Color _getRiskColor(String riskCategory) {
    switch (riskCategory) {
      ...
      case 'Tidak Ada Risiko':
        return Colors.green;
      default:
        return const Color(0xFF64D1DE); // Blue color for prostate cancer theme
    }
  }
  ...
  Widget _buildResultCard(Map<String, dynamic> result) {
    ...
    final Map<String, String>? info = PenyakitProstat.penyakitInfo[penyakit];
