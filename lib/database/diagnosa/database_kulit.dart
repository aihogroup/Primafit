...
class DatabaseHelper with DiagnosisHistoryMixin {
  ...
  Future<Database> get database async {
    ...
    _database = await _initDB('kulit_diagnosa.db');
    return _database!;
  }
  ...
  Future _createDB(Database db, int version) async {
    // Tabel untuk menyimpan data penyakit dan gejala kulit
    await db.execute('''
    CREATE TABLE penyakit_kulit (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      nama_penyakit TEXT NOT NULL,
      gatal TEXT,
      perubahan_warna TEXT,
      luka TEXT,
      kemerahan TEXT,
      mengelupas TEXT,
      kering TEXT,
  ...
  Future<bool> importCSV() async {
    try {
      ...
      final count = Sqflite.firstIntValue(await db.rawQuery('SELECT COUNT(*) FROM penyakit_kulit'));
      ...
      for (var header in csvData[0]) {
        ...
        // Khusus untuk kolom pertama, ganti '' menjadi 'nama_penyakit'
        if (normalizedHeader == '') {
      ...
      for (int i = 1; i < csvData.length; i++) {
        ...
        for (int j = 0; j < headers.length && j < row.length; j++) {
          ...
          if (value.toLowerCase() == 'ya') value = 'Ya';
        ...
        batch.insert('penyakit_kulit', rowData);
        importedRows++;
      }
  ...
  Future<int> saveUserAnswers(Map<String, dynamic> answers) async {
    ...
    try {
      // Cek dahulu apakah kolom created_at ada dalam tabel user_answers
      var tableInfo = await db.rawQuery("PRAGMA table_info(user_answers)");
      bool hasCreatedAt = tableInfo.any((column) => column['name'] == 'created_at');
      
      // Jika kolom created_at tidak ada, tambahkan kolom tersebut
      if (!hasCreatedAt) {
        debugPrint('Kolom created_at tidak ditemukan, menambahkan kolom...');
        await db.execute('ALTER TABLE user_answers ADD COLUMN created_at TEXT');
      }
  ...
  Future<List<Map<String, dynamic>>> getAllDiseases() async {
    final db = await database;
    return await db.query('penyakit_kulit');
  }
  ...
  Future<Map<String, dynamic>?> getLatestUserAnswers() async {
    ...
    // Query yang berbeda tergantung ada tidaknya kolom created_at
    final List<Map<String, dynamic>> results = hasCreatedAt 
        ? await db.query(
            'user_answers',
  ...
  Future<List<Map<String, dynamic>>> getDiagnosisResult(double threshold) async {
    ...
    // Daftar kolom gejala utama
    final List<String> symptomColumns = [
      'gatal', 'perubahan_warna', 'luka', 'kemerahan', 'mengelupas',
      'kering', 'kebas', 'bengkak', 'benjolan', 'nyeri',
      'rambut_rontok', 'sensitif', 'bekas_luka', 'infeksi_jamur', 'perubahan_sensasi'
    ];
    ...
    for (var disease in diseases) {
      ...
      Map<String, bool> matchedDetails = {};
      
      // Periksa setiap gejala
      for (var symptom in symptomColumns) {
        // Jika data gejala tersedia untuk penyakit ini
        if (disease[symptom] != null) {
          String diseaseValue = disease[symptom].toString().trim();
          String userValue = validUserAnswers[symptom] ?? 'Tidak';
      ...
      double matchPercentage = totalSymptoms > 0 
    ...
    if (diagnosisResults.isEmpty && diseases.isNotEmpty) {
      ...
      for (var disease in diseases) {
        ...
        double percentage = total > 0 ? (matched / total) * 100 : 0;
  ...
  Future<List<Map<String, dynamic>>> getWeightedDiagnosisResult(double threshold) async {
    ...
    for (var disease in diseases) {
      ...
      Map<String, bool> matchedSymptomDetails = {};
      ...
      for (var symptom in symptomWeights.keys) {
        ...
        if (disease[symptom] != null) {
          ...
          // Hanya perhatikan gejala yang relevan (bukan nilai kosong atau default)
          if (diseaseValue.isNotEmpty && diseaseValue != '-') {
            double weight = symptomWeights[symptom] ?? 0.5;  // Default weight if not specified
            totalWeight += weight;
            totalSymptoms++;
      ...
      double weightedPercentage = totalWeight > 0 
      ...
      for (var disease in diseases) {
        ...
        for (var symptom in symptomWeights.keys) {
          if (disease[symptom] != null && disease[symptom].toString().trim().isNotEmpty && disease[symptom].toString().trim() != '-') {
            double weight = symptomWeights[symptom] ?? 0.5;
            totalWeight += weight;
            total++;
        ...
        double percentage = totalWeight > 0 ? (matchedWeight / totalWeight) * 100 : 0;
        if (percentage > highestPercentage) {
          highestPercentage = percentage;
          bestMatch = {
            'penyakit': disease['nama_penyakit'],
