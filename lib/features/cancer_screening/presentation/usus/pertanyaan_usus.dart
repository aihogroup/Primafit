...
import 'package:primafit/features/cancer_screening/presentation/usus/output_usus.dart';
import 'package:primafit/features/cancer_screening/presentation/usus/list_pertanyaan.dart';

class KankerUsusPage extends StatefulWidget {
  const KankerUsusPage({super.key});

  @override
  _KankerUsusPageState createState() => _KankerUsusPageState();
}

class _KankerUsusPageState extends State<KankerUsusPage> with SingleTickerProviderStateMixin {
  ...
  void _nextPage() {
    if (_currentPage < KankerUsusQuestions.activeQuestions(_answers).length - 1) {
      _animateToNextPage();
    } else {
      _submitAnswers();
    }
  ...
  Future<void> _submitAnswers() async {
    ...
    try {
      ...
      for (var question in KankerUsusQuestions.questions) {
        final String id = question['id'];
        
        // Check if this question should be skipped
        bool shouldSkip = false;
        for (var otherQuestion in KankerUsusQuestions.questions) {
      ...
      await DatabaseHelperUsus.instance.saveUserAnswers(userAnswers);
      ...
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(builder: (context) => const OutputPageUsus()),
      );
    } catch (e) {
      if (!mounted) return;
          ...
          onTap: () {
            setState(() {
              ...
              if (answer == 'Ya') {
                ...
                final currentQuestion = KankerUsusQuestions.questions.firstWhere(
                  (q) => q['id'] == questionId,
                  orElse: () => {},
                );
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Text(
                      'Pertanyaan ${index + 1}/${KankerUsusQuestions.activeQuestions(_answers).length}',
                      style: GoogleFonts.poppins(
                        fontSize: 14,
                        fontWeight: FontWeight.w500,
                        color: Colors.grey.shade700,
                      ...
                      child: Stack(
                        children: [
                          AnimatedFractionallySizedBox(
                            duration: const Duration(milliseconds: 300),
                            widthFactor: (index + 1) / KankerUsusQuestions.activeQuestions(_answers).length,
                            child: Container(
                              decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(10),
                ...
                Row(
                  ...
                  children: [
                    ...
                    if (index < KankerUsusQuestions.activeQuestions(_answers).length - 1)
                      ElevatedButton.icon(
  ...
  Widget build(BuildContext context) {
    ...
    final activeQuestions = KankerUsusQuestions.activeQuestions(_answers);
