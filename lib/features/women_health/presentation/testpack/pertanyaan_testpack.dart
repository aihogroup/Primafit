...
import 'dart:async';
import 'package:primafit/features/women_health/data/database_testpack.dart'; // Sesuaikan dengan path database Anda
import 'package:primafit/features/women_health/presentation/testpack/output_testpack.dart'; // Sesuaikan dengan path output Anda
import 'package:primafit/features/women_health/presentation/testpack/list_pertanyaan.dart'; // Sesuaikan dengan path list pertanyaan Anda

class KehamilanPage extends StatefulWidget {
  const KehamilanPage({super.key});

  @override
  _KehamilanPageState createState() => _KehamilanPageState();
}

class _KehamilanPageState extends State<KehamilanPage> with SingleTickerProviderStateMixin {
  ...
  void _nextPage() {
    if (_currentPage < KehangilanQuestions.activeQuestions(_answers).length - 1) {
      _animateToNextPage();
    } else {
      _submitAnswers();
    }
  ...
  Future<void> _submitAnswers() async {
    ...
    try {
      ...
      for (var question in KehangilanQuestions.questions) {
        final String id = question['id'];
        
        // Check if this question should be skipped
        bool shouldSkip = false;
        for (var otherQuestion in KehangilanQuestions.questions) {
      ...
      await DatabaseHelperKehamilan.instance.saveUserAnswers(userAnswers);
      ...
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(builder: (context) => const OutputPageKehamilan()),
      );
    } catch (e) {
      if (!mounted) return;
  ...
  Widget _buildAnswerButton(String answer, String questionId) {
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: isSelected ? const Color(0xFFE9458D) : Colors.grey.shade300,
      ...
      child: Material(
        ...
        child: InkWell(
          borderRadius: BorderRadius.circular(12),
          splashColor: const Color(0xFFE9458D).withValues(alpha: 0.1),
          highlightColor: const Color(0xFFE9458D).withValues(alpha: 0.05),
          onTap: () {
            setState(() {
              ...
              if (answer == 'Ya') {
                ...
                final currentQuestion = KehangilanQuestions.questions.firstWhere(
                  (q) => q['id'] == questionId,
                  orElse: () => {},
                );
              ...
              children: [
                Text(
                  ...
                  style: GoogleFonts.poppins(
                    ...
                    color: isSelected ? const Color(0xFFE9458D) : Colors.black87,
                  ),
                ),
                AnimatedOpacity(
                  duration: const Duration(milliseconds: 300),
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Text(
                      'Pertanyaan ${index + 1}/${KehangilanQuestions.activeQuestions(_answers).length}',
                      style: GoogleFonts.poppins(
                        fontSize: 14,
                        fontWeight: FontWeight.w500,
                        color: Colors.grey.shade700,
                      ...
                      child: Stack(
                        children: [
                          AnimatedFractionallySizedBox(
                            duration: const Duration(milliseconds: 300),
                            widthFactor: (index + 1) / KehangilanQuestions.activeQuestions(_answers).length,
                            child: Container(
                              decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(10),
                                ...
                                boxShadow: [
                                  BoxShadow(
                                    color: const Color(0xFFE9458D).withValues(alpha: 0.4),
                                    blurRadius: 3,
                ...
                Row(
                  ...
                  children: [
                    ...
                    if (index < KehangilanQuestions.activeQuestions(_answers).length - 1)
                      ElevatedButton.icon(
  ...
  Widget build(BuildContext context) {
    ...
    final activeQuestions = KehangilanQuestions.activeQuestions(_answers);
