...
class UpdateTensiScreen extends StatefulWidget {
  final int TensiId;
  
  const UpdateTensiScreen({
    super.key,
    required this.TensiId,
  });

  @override
  State<UpdateTensiScreen> createState() => _UpdateTensiScreenState();
}

class _UpdateTensiScreenState extends State<UpdateTensiScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadTensiData() async {
    try {
      final data = await TekananDarahDatabaseHelper.instance.getTekananDarahById(widget.TensiId);
      if (data != null) {
        setState(() {
          _currentData = data;
          _sistolikController.text = data.sistolik;
          _diastolikController.text = data.diastolik;
  ...
  void _updateData() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        final updatedTensi = TekananDarah(
          id: widget.TensiId,
          tanggal: formattedDate,
          waktu: formattedTime,
          sistolik: _sistolikController.text,
          diastolik: _diastolikController.text,
          catatan: _catatanController.text,
        );
        
        await TekananDarahDatabaseHelper.instance.updateTekananDarah(updatedTensi);
                  ...
                  child: Padding(
                    ...
                    child: Form(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            'Perbarui data pemeriksaan Tensi Anda',
                            style: GoogleFonts.poppins(
                              fontSize: 14,
                          ...
                          const SizedBox(height: 16),
                          Text(
                            'Hasil Periksa Kadar Tensi Total',
                            style: GoogleFonts.poppins(
                              fontSize: 16,
                              fontWeight: FontWeight.w500,
                            ),
