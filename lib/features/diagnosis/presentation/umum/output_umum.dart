...
import 'package:primafit/features/diagnosis/data/database_umum.dart';
...
import 'dart:io';
import 'package:primafit/features/diagnosis/presentation/umum/list_penyakit.dart';
...
class _OutputPageState extends State<OutputPage> with SingleTickerProviderStateMixin {
  ...
  Future<File> _generatePDF() async {
        ...
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              ...
              pw.SizedBox(height: 20),
              if (_diagnosisResults.isNotEmpty && PenyakitUmum.penyakitInfo.containsKey(_diagnosisResults[0]['penyakit']))
                pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text(
                    ...
                    pw.SizedBox(height: 5),
                    pw.Text(
                      PenyakitUmum.penyakitInfo[_diagnosisResults[0]['penyakit']]!['description'] ?? '',
                    ),
                    ...
                    pw.SizedBox(height: 5),
                    pw.Text(
                      PenyakitUmum.penyakitInfo[_diagnosisResults[0]['penyakit']]!['treatment'] ?? '',
                    ),
                  ],
                ),
            ],
    ...
    final file = File('${output.path}/hasil_diagnosa_umum.pdf');
  ...
  Future<void> _shareResults() async {
    try {
      ...
      await Share.shareXFiles(
        [XFile(pdfFile.path)],
        text: 'Hasil Diagnosa Penyakit Umum',
        subject: 'Hasil Diagnosa Penyakit Umum - ${DateFormat('dd MMM yyyy').format(DateTime.now())}',
      );
    } catch (e) {
      if (!mounted) return;
  ...
  Widget _buildResultCard(Map<String, dynamic> result) {
    ...
    final Map<String, String>? info = PenyakitUmum.penyakitInfo[penyakit];
