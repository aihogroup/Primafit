...
import 'package:primafit/features/skincare/presentation/exfoliator/output_exfoliator.dart';
import 'package:primafit/features/skincare/presentation/exfoliator/list_pertanyaan.dart';

class ExfoliatorRecommendationPage extends StatefulWidget {
  const ExfoliatorRecommendationPage({super.key});

  @override
  _ExfoliatorRecommendationPageState createState() => _ExfoliatorRecommendationPageState();
}

class _ExfoliatorRecommendationPageState extends State<ExfoliatorRecommendationPage> with SingleTickerProviderStateMixin {
  ...
  void _nextPage() {
    if (_currentPage < ExfoliatorQuestions.activeQuestions(_answers).length - 1) {
      _animateToNextPage();
    } else {
      _submitAnswers();
    }
  ...
  Future<void> _submitAnswers() async {
    ...
    try {
      ...
      final userAnswerId = await DatabaseHelperExfoliator.instance.saveUserAnswers(userAnswers);

      // Hitung preferensi produk berdasarkan jawaban
      final productPreference = ExfoliatorQuestions.calculateProductPreference(_answers);
      
      // Simpan hasil rekomendasi
      await DatabaseHelperExfoliator.instance.saveRecommendationResult(userAnswerId, productPreference);
      ...
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(builder: (context) => const OutputPageExfoliator()),
      );
    } catch (e) {
      if (!mounted) return;
  ...
  Widget _buildQuestionPage(Map<String, dynamic> question, int index) {
    ...
    final List<String> options = ExfoliatorQuestions.getOptionsForQuestion(questionId, answers: _answers);
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Text(
                      'Pertanyaan ${index + 1}/${ExfoliatorQuestions.activeQuestions(_answers).length}',
                      style: GoogleFonts.poppins(
                        fontSize: 14,
                        fontWeight: FontWeight.w500,
                        color: Colors.grey.shade700,
                      ...
                      child: Stack(
                        children: [
                          AnimatedFractionallySizedBox(
                            duration: const Duration(milliseconds: 300),
                            widthFactor: (index + 1) / ExfoliatorQuestions.activeQuestions(_answers).length,
                            child: Container(
                              decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(10),
                ...
                Row(
                  ...
                  children: [
                    ...
                    if (index < ExfoliatorQuestions.activeQuestions(_answers).length - 1)
                      ElevatedButton.icon(
  ...
  Widget build(BuildContext context) {
    ...
    final activeQuestions = ExfoliatorQuestions.activeQuestions(_answers);
