...
class DatabaseHelper with DiagnosisHistoryMixin {
  ...
  Future<Database> get database async {
    ...
    _database = await _initDB('paru_diagnosa.db');
    return _database!;
  }
  ...
  Future _createDB(Database db, int version) async {
    ...
    CREATE TABLE penyakit_paru (
      ...
      batuk_kronis TEXT,
      sesak_nafas TEXT,
      sesak_saat_istirahat TEXT,
      sesak_saat_berbaring TEXT,
      nyeri_dada TEXT,
      nyeri_pleuritik TEXT,
      bunyi_nafas TEXT,
      bunyi_mengi TEXT,
      bunyi_ronchi TEXT,
      penurunan_bb_drastis TEXT,
      demam TEXT,
      demam_pola TEXT,
      keringat_malam TEXT,
      kelelahan_berkepanjangan TEXT,
      sianosis TEXT
    )
    ''');
  ...
  Future<bool> importCSV() async {
    try {
      ...
      try {
        csvString = await rootBundle.loadString('assets/csv/paru_paru.csv');
      ...
      for (int i = 1; i < csvData.length; i++) {
        ...
        batch.insert('penyakit_paru', rowData);
        importedRows++;
      }
  ...
  Future<List<Map<String, dynamic>>> getDiagnosisResult(double threshold) async {
    ...
    final List<String> symptomColumns = [
      'batuk', 'batuk_berdahak', 'dahak_kuning', 'dahak_hijau', 'batuk_berdarah',
      'batuk_kronis', 'sesak_nafas', 'sesak_saat_istirahat', 'sesak_saat_berbaring',
      'nyeri_dada', 'nyeri_pleuritik', 'bunyi_nafas', 'bunyi_mengi', 'bunyi_ronchi',
      'penurunan_bb_drastis', 'demam', 'demam_pola', 'keringat_malam',
      'kelelahan_berkepanjangan', 'sianosis'
    ];
  ...
  Future<List<Map<String, dynamic>>> getWeightedDiagnosisResult(double threshold) async {
    ...
    final Map<String, double> symptomWeights = {
      'batuk': 0.5,  // Gejala umum, bobot rendah
      'batuk_berdahak': 0.7,
      'dahak_kuning': 0.8,
      'dahak_hijau': 0.8,
      'batuk_berdarah': 1.0,  // Gejala spesifik, bobot tinggi
      'batuk_kronis': 0.9,
      'sesak_nafas': 0.7,
      'sesak_saat_istirahat': 0.9,
      'sesak_saat_berbaring': 0.9,
      'nyeri_dada': 0.8,
      'nyeri_pleuritik': 0.9,
      'bunyi_nafas': 0.7,
      'bunyi_mengi': 0.8,
      'bunyi_ronchi': 0.8,
      'penurunan_bb_drastis': 0.8,
      'demam': 0.6,
      'demam_pola': 0.8,
      'keringat_malam': 0.9,
      'kelelahan_berkepanjangan': 0.7,
      'sianosis': 1.0  // Gejala sangat spesifik
    };
