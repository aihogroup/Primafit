-- =============================================================================
-- Sprint 1 · Verification: resubmission after rejection + mandatory review note
-- -----------------------------------------------------------------------------
-- Changes to private.guard_verification():
--   * owner edits a REJECTED registration  -> automatically back to 'pending'
--     (resubmission); the previous note is cleared (kept in audit_logs)
--   * superadmin rejects or suspends       -> verification_note is required
-- Everything else is unchanged (see 20260924130326_identity_rbac.sql).
-- Rollback: supabase/rollbacks/20260924142210_verification_resubmission.down.sql
-- =============================================================================
...
begin
  if not is_admin then
    ...
    if new.verification_status in ('rejected', 'suspended')
       and nullif(trim(new.verification_note), '') is null then
