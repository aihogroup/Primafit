...
import 'dart:async';
import 'package:primafit/features/cancer_screening/data/database_paru.dart';
import 'package:primafit/features/cancer_screening/presentation/paru/output_paru.dart';
import 'package:primafit/features/cancer_screening/presentation/paru/list_pertanyaan.dart';

class KankerParuPage extends StatefulWidget {
  const KankerParuPage({super.key});

  @override
  _KankerParuPageState createState() => _KankerParuPageState();
}

class _KankerParuPageState extends State<KankerParuPage> with SingleTickerProviderStateMixin {
  ...
  void _nextPage() {
    if (_currentPage < KankerParuQuestions.activeQuestions(_answers).length - 1) {
      _animateToNextPage();
    } else {
      _submitAnswers();
    }
  ...
  Future<void> _submitAnswers() async {
    ...
    try {
      ...
      for (var question in KankerParuQuestions.questions) {
        final String id = question['id'];
        
        // Check if this question should be skipped
        bool shouldSkip = false;
        for (var otherQuestion in KankerParuQuestions.questions) {
        ...
        child: InkWell(
          ...
          onTap: () {
            setState(() {
              ...
              // e.g., if perokok_aktif is "Ya", we'll skip bekas_perokok automatically
              if (answer == 'Ya') {
                // Find the current question
                final currentQuestion = KankerParuQuestions.questions.firstWhere(
                  (q) => q['id'] == questionId,
                  orElse: () => {},
                );
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Text(
                      'Pertanyaan ${index + 1}/${KankerParuQuestions.activeQuestions(_answers).length}',
                      style: GoogleFonts.poppins(
                        fontSize: 14,
                        fontWeight: FontWeight.w500,
                        color: Colors.grey.shade700,
                      ...
                      child: Stack(
                        children: [
                          AnimatedFractionallySizedBox(
                            duration: const Duration(milliseconds: 300),
                            widthFactor: (index + 1) / KankerParuQuestions.activeQuestions(_answers).length,
                            child: Container(
                              decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(10),
                ...
                Row(
                  ...
                  children: [
                    ...
                    if (index < KankerParuQuestions.activeQuestions(_answers).length - 1)
                      ElevatedButton.icon(
  ...
  Widget build(BuildContext context) {
    ...
    final activeQuestions = KankerParuQuestions.activeQuestions(_answers);
