...
class AlarmService {
  ...
  void _onNotificationTapped(NotificationResponse response) async {
    if (response.payload != null) {
      ...
      debugPrint('Notification tapped with obat ID: $obatId');
  ...
  Future<void> _saveAlarmToPrefs(int alarmId, Map<String, dynamic> data) async {
    try {
      ...
      debugPrint('Error saving alarm to prefs: $e');
    }
  }
  ...
  Future<Map<String, dynamic>?> _getAlarmFromPrefs(int alarmId) async {
    try {
      ...
      debugPrint('Error getting alarm from prefs: $e');
      return null;
    }
  }
  ...
  try {
    ...
    for (var alarm in alarms) {
      ...
      if (rawAlarmId == null) {
        debugPrint('Alarm ID is null, skipping...');
        continue;
      }

      final int alarmId = int.tryParse(rawAlarmId.toString()) ?? -1;
      if (alarmId == -1) {
        debugPrint('Invalid alarm ID: $rawAlarmId, skipping...');
        continue;
      }
  ...
  Future<void> _removeAlarmFromPrefs(int alarmId) async {
    try {
      ...
      debugPrint('Error removing alarm from prefs: $e');
    }
  }
  ...
  static Future<void> _handleAlarmDirectly(int id) async {
    try {
      ...
      debugPrint('Error handling alarm directly: $e');
    }
  }
  ...
  Future<void> checkAndUpdateAlarms() async {
    try {
      ...
      debugPrint('Error checking and updating alarms: $e');
    }
  }
}