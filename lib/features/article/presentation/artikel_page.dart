...
import 'package:primafit/features/article/presentation/detail_artikel.dart';
import 'package:primafit/core/widgets/app_bottom_navigation.dart';
import 'dart:math' as math;
import 'package:primafit/features/article/data/database_artikel.dart'; // Import database
...
void initState() {
  ...
  Future<void> fetchArticles() async {
    ...
    if (!Env.isNewsApiConfigured) {
      setState(() {
        errorMessage = 'Artikel belum tersedia. Konfigurasi NEWS_API_KEY belum diatur.';
        isLoading = false;
      });
      return;
    }
    ...
    try {
      // Key goes in a header, not the URL, so it never appears in request logs.
      final response = await http.get(
        Uri.https('newsapi.org', '/v2/everything', {
          'domains': 'detik.com,liputan6.com',
          'q': 'kesehatan',
        }),
      ...
      if (response.statusCode == 200) {
        ...
        AppLogger.warning('NewsAPI responded ${response.statusCode}', tag: 'article');
        setState(() {
          errorMessage = 'Gagal memuat artikel (kode ${response.statusCode}). Coba lagi nanti.';
          isLoading = false;
        });
      }
    } catch (e) {
      AppLogger.warning('NewsAPI request failed', tag: 'article', error: e);
      if (!mounted) return;
      setState(() {
  ...
  void updateFilteredArticles() {
    ...
    // Apply sort
    result.sort((a, b) {
      final DateTime dateA = DateTime.parse(a['publishedAt'] ?? DateTime.now().toIso8601String());
      final DateTime dateB = DateTime.parse(b['publishedAt'] ?? DateTime.now().toIso8601String());
                ...
                child: Row(
                  children: [
                    ...
                    Switch.adaptive(
                      ...
                      activeThumbColor: const Color(0xFF64D1DE),
                      onChanged: (value) {
                        setState(() {
