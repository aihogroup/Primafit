...
import 'package:primafit/database/diagnosa/database_mental.dart';
...
import 'dart:io';
import 'package:primafit/UI/diagnosa/mental/list_penyakit.dart';
...
class _OutputPageState extends State<OutputPage> with SingleTickerProviderStateMixin {
  ...
  Future<File> _generatePDF() async {
        ...
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              pw.Center(
                child: pw.Text(
                  'HASIL DIAGNOSA PENYAKIT MENTAL',
                  style: pw.TextStyle(
                    fontSize: 18, 
              ...
              pw.SizedBox(height: 20),
              if (_diagnosisResults.isNotEmpty && PenyakitMental.penyakitInfo.containsKey(_diagnosisResults[0]['penyakit']))
                pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text(
                    ...
                    pw.SizedBox(height: 5),
                    pw.Text(
                      PenyakitMental.penyakitInfo[_diagnosisResults[0]['penyakit']]!['description'] ?? '',
                    ),
                    ...
                    pw.SizedBox(height: 5),
                    pw.Text(
                      PenyakitMental.penyakitInfo[_diagnosisResults[0]['penyakit']]!['treatment'] ?? '',
                    ),
                  ],
                ),
            ],
    ...
    final file = File('${output.path}/hasil_diagnosa_mental.pdf');
  ...
  Future<void> _shareResults() async {
    try {
      ...
      await Share.shareXFiles(
        [XFile(pdfFile.path)],
        text: 'Hasil Diagnosa Penyakit Mental',
        subject: 'Hasil Diagnosa Penyakit Mental - ${DateFormat('dd MMM yyyy').format(DateTime.now())}',
      );
    } catch (e) {
  ...
  Widget _buildResultCard(Map<String, dynamic> result) {
    ...
    final Map<String, String>? info = PenyakitMental.penyakitInfo[penyakit];
