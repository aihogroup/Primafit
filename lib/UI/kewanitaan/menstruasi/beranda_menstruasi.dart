...
import 'package:primafit/UI/kewanitaan/menstruasi/model_menstruasi.dart';
import 'package:primafit/database/kewanitaan/database_menstruasi.dart';
import 'package:primafit/UI/kewanitaan/menstruasi/tambah_siklus.dart';
import 'package:primafit/UI/kewanitaan/menstruasi/tambah_gejala.dart';
import 'package:primafit/UI/kewanitaan/menstruasi/statistik_siklus.dart';
import 'package:primafit/UI/kewanitaan/menstruasi/pengaturan_siklus.dart';

class MenstrualTrackerHomePage extends StatefulWidget {
  const MenstrualTrackerHomePage({Key? key}) : super(key: key);
...
class _MenstrualTrackerHomePageState extends State<MenstrualTrackerHomePage> with SingleTickerProviderStateMixin {
  ...
  List<Widget> _getEventsForDay(DateTime day) {
    ...
    // Check for period, ovulation, or fertile days
    bool inPeriod = false;
    ...
    for (var cycle in _cycles) {
      ...
      if (cycle.isOvulation(day)) {
        isOvulationDay = true;
      }
      
      if (cycle.isFertile(day)) {
        isFertileDay = true;
      }
    }
    ...
    if (inPeriod) {
      events.add(
        _buildEventChip('Menstruasi', const Color(0xFFE9458D), CupertinoIcons.drop_fill)
      );
    }
    
    if (isOvulationDay) {
      events.add(
        _buildEventChip('Ovulasi', const Color(0xFFFFC400), CupertinoIcons.star_fill)
      );
    } else if (isFertileDay && !inPeriod) {
      events.add(
        _buildEventChip('Masa Subur', const Color(0xFF8BC34A), CupertinoIcons.leaf_arrow_circlepath)
      );
    }
    ...
    if (_symptoms.containsKey(dateOnly)) {
      final daySymptoms = _symptoms[dateOnly]!;
      
      // Group symptoms by type and show count if multiple
      final Map<SymptomType, int> symptomCounts = {};
      for (var symptom in daySymptoms) {
        if (symptomCounts.containsKey(symptom.type)) {
          symptomCounts[symptom.type] = symptomCounts[symptom.type]! + 1;
        } else {
          symptomCounts[symptom.type] = 1;
        }
      }
      
      // Show up to 2 symptom types in the calendar
      int count = 0;
      symptomCounts.forEach((type, number) {
        if (count < 2) {
          events.add(
            _buildEventChip(
              type.displayName, 
      ...
      if (symptomCounts.length > 2) {
        events.add(
          _buildEventChip(
            '+${symptomCounts.length - 2}', 
    ...
    return TableCalendar(
      ...
      calendarBuilders: CalendarBuilders(
        ...
        defaultBuilder: (context, day, focusedDay) {
          ...
          for (var cycle in _cycles) {
            if (cycle.isInPeriod(day)) {
              cellColor = const Color(0xFFE9458D).withOpacity(0.2);
              break;
            } else if (cycle.isOvulation(day)) {
              cellColor = const Color(0xFFFFC400).withOpacity(0.2);
              break;
            } else if (cycle.isFertile(day)) {
              cellColor = const Color(0xFF8BC34A).withOpacity(0.1);
              break;
            }
          }
