...
class DatabaseHelperPayudara {
  static final DatabaseHelperPayudara instance = DatabaseHelperPayudara._init();
  static Database? _database;

  DatabaseHelperPayudara._init();
  ...
  Future<bool> addSampleData() async {
    try {
      ...
      final count = Sqflite.firstIntValue(await db.rawQuery('SELECT COUNT(*) FROM penyakit_payudara'));
      ...
      for (var category in riskCategories) {
        batch.insert('penyakit_payudara', category);
      }
  ...
  Future _createDB(Database db, int version) async {
    ...
    await db.execute('''
    CREATE TABLE penyakit_payudara (
    ...
    await db.execute('''
    CREATE TABLE user_answers_payudara (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      riwayat_keluarga TEXT,
      pernah_kanker TEXT,
      menstruasi_dini TEXT,
  ...
  Future<int> saveUserAnswers(Map<String, dynamic> answers) async {
    ...
    try {
      ...
      await db.transaction((txn) async {
        ...
        await txn.delete('user_answers_payudara');
        
        // Simpan jawaban baru
        insertId = await txn.insert('user_answers_payudara', normalizedAnswers);
      });
  ...
  Future<List<Map<String, dynamic>>> getAllDiseases() async {
    final db = await database;
    return await db.query('penyakit_payudara');
  }
