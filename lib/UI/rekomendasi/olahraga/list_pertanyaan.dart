...
class ExerciseQuestions {
  ...
  static Map<String, dynamic> calculateExerciseNeeds(Map<String, dynamic> answers) {
    ...
    try {
      ...
      // Get basic info
      String usia = answers['usia'] ?? 'Dewasa (19-50 tahun)';
      String jenisKelamin = answers['jenis_kelamin'] ?? 'Laki-laki';
      String tingkatAktivitas = answers['tingkat_aktivitas'] ?? 'Sedang (olahraga moderat 3-5 hari/minggu)';
      String tujuanOlahraga = answers['tujuan_olahraga'] ?? 'Pemeliharaan kebugaran umum';
      String kondisiKesehatan = answers['kondisi_kesehatan'] ?? 'Tidak ada';
      String levelKebugaran = answers['level_kebugaran'] ?? 'Menengah (rutin berolahraga 3-4x/minggu)';
      String waktuTersedia = answers['waktu_tersedia'] ?? 'Sedang (30-60 menit/hari)';
      ...
      double activityMultiplier = getActivityMultiplier(tingkatAktivitas);
      double tdee = bmr * activityMultiplier;
      
      // Calculate exercise parameters
      int recommendedMinutes = calculateRecommendedExerciseMinutes(waktuTersedia, tujuanOlahraga, levelKebugaran, usia);
      int recommendedFrequency = calculateRecommendedFrequency(tujuanOlahraga, levelKebugaran, usia);
      String recommendedIntensity = calculateRecommendedIntensity(kondisiKesehatan, tujuanOlahraga, levelKebugaran, bmi, usia);
      int recoveryTime = calculateRecoveryTime(usia, levelKebugaran, tujuanOlahraga);
      
      // Calculate target heart rate based on age
      Map<String, int> targetHeartRate = calculateTargetHeartRate(getEstimatedAgeValue(usia), recommendedIntensity);
      
      // Estimate calories burned (rough estimate)
      int caloriesBurnEstimate = estimateCaloriesBurn(beratBadan, recommendedMinutes, recommendedIntensity);
      
      // Determine exercise level
      String exerciseLevel = determineExerciseLevel(levelKebugaran, tingkatAktivitas, bmi, usia);
      
      // Determine main category for recommendations
      String mainCategory = determineMainCategory(usia, kondisiKesehatan, answers);
      String secondaryCategory = determineSecondaryCategory(tujuanOlahraga, answers);
  ...
  static int calculateRecommendedExerciseMinutes(String availableTime, String goal, String fitnessLevel, String ageCategory) {
    ...
    int finalMinutes = max(10, (baseMinutes * goalMultiplier * fitnessMultiplier * ageMultiplier).round());
  ...
  static String calculateRecommendedIntensity(String healthCondition, String goal, String fitnessLevel, double bmi, String ageCategory) {
    ...
    switch (fitnessLevel) {
      ...
      case 'Dasar (kadang-kadang berolahraga)':
        ...
        break;
      case 'Atlet (latihan terstruktur intensif)':
        if (baseIntensity == 'Moderate') adjustedIntensity = 'Moderate-to-High';
  ...
  static int estimateCaloriesBurn(double weightKg, int minutes, String intensity) {
    ...
    int calories = (met * weightKg * hours).round();
  ...
  static String determineMainCategory(String ageCategory, String healthCondition, Map<String, dynamic> answers) {
    ...
    switch (ageCategory) {
      ...
      case 'Dewasa (19-50 tahun)':
        ...
        String tingkatAktivitas = answers['tingkat_aktivitas'] ?? '';
  ...
  static String determineSecondaryCategory(String goal, Map<String, dynamic> answers) {
    String preferensiOlahraga = answers['preferensi_olahraga'] ?? 'Tidak memiliki preferensi khusus';
