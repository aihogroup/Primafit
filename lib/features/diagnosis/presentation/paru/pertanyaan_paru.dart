...
import 'package:primafit/features/diagnosis/presentation/paru/output_paru.dart';
import 'package:primafit/features/diagnosis/presentation/paru/list_pertanyaan.dart';

class ParuPage extends StatefulWidget {
  const ParuPage({super.key});

  @override
  _ParuPageState createState() => _ParuPageState();
}

class _ParuPageState extends State<ParuPage> with SingleTickerProviderStateMixin {
  ...
  void _nextPage() {
    if (_currentPage < ParuQuestions.activeQuestions(_answers).length - 1) {
      _animateToNextPage();
    } else {
      _submitAnswers();
    }
  ...
  Future<void> _submitAnswers() async {
    ...
    for (var question in ParuQuestions.questions) {
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  Text(
                    'Pertanyaan ${index + 1}/${ParuQuestions.activeQuestions(_answers).length}',
                    style: GoogleFonts.poppins(
                      fontSize: 14,
                      fontWeight: FontWeight.w500,
                      color: Colors.grey.shade700,
                    ...
                    child: Stack(
                      children: [
                        AnimatedFractionallySizedBox(
                          duration: const Duration(milliseconds: 300),
                          widthFactor: (index + 1) / ParuQuestions.activeQuestions(_answers).length,
                          child: Container(
                            decoration: BoxDecoration(
                              borderRadius: BorderRadius.circular(10),
              ...
              Row(
                ...
                children: [
                  ...
                  if (index < ParuQuestions.activeQuestions(_answers).length - 1)
                    ElevatedButton.icon(
      ...
      body: _loading
          ...
          : SafeArea(
              child: PageView.builder(
                ...
                itemCount: ParuQuestions.activeQuestions(_answers).length,
                itemBuilder: (context, index) {
                  return _buildQuestionPage(ParuQuestions.activeQuestions(_answers)[index], index);
                },
              ),
            ),
    );
