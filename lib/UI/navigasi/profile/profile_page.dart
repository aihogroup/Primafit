...
import 'package:primafit/UI/catat/bmi/read_bmi.dart';
...
import 'package:primafit/UI/catat/tensi/read_tensi.dart';
...
class _ProfilePageState extends State<ProfilePage> with SingleTickerProviderStateMixin {
  ...
  print("Tanggal lahir dari DB: ${_profileData['tanggalLahir']}");
    setState(() {
      _isLoading = true;
    });
    ...
    try {
      ...
      if (profiles.isNotEmpty) {
        setState(() {
          ...
          _namaController.text = _profileData['nama'] ?? '';
          _tanggalLahirController.text = _profileData['tanggalLahir'] ?? '';
          _teleponController.text = _profileData['telepon'] ?? '';
          _selectedGender = _profileData['gender'];
          _selectedGolonganDarah = _profileData['golonganDarah'];
          _profileImagePath = _profileData['foto'];
        });
      }
    } catch (e) {
      _showErrorMessage('Gagal memuat data profil: $e');
    } finally {
      setState(() {
        _isLoading = false;
      });
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      if (_profileData.containsKey('id')) {
        await _databaseHelper.updateProfile(profile, _profileData['id']);
      } else {
      ...
      _showErrorMessage('Gagal menyimpan profil: $e');
    } finally {
      setState(() {
        _isLoading = false;
      });
