...
import 'package:primafit/database/kewanitaan/database_parenting.dart';
...
class _GrowthChartPageState extends State<GrowthChartPage>
  ...
  List<Map<String, dynamic>> _getFilteredRecords() {
    List<Map<String, dynamic>> filtered = List.from(_growthRecords);
    filtered.sort((a, b) => 
  ...
  Widget _buildSummaryStats(bool isTablet) {
    ...
    final headCircumference = _latestRecord!['head_circumference']?.toDouble() ?? 0.0;
            ...
            sideTitles: SideTitles(
              ...
              getTitlesWidget: (double value, TitleMeta meta) {
                ...
                if (value.toInt() >= 0 && value.toInt() < records.length) {
                  final date = DateTime.parse(records[value.toInt()]['record_date']);
  ...
  String _getGrowthStatus(double? weight, double? height, int ageInMonths, String gender) {
    // This is a simplified version. In a real app, you would use WHO growth standards
    if (weight == null && height == null) return 'Data tidak lengkap';
    
    // Sample growth status logic (should be replaced with proper WHO percentiles)
    if (weight != null) {
      double expectedWeight = _getExpectedWeight(ageInMonths, gender);
      double percentage = (weight / expectedWeight) * 100;
  ...
  double _getExpectedWeight(int ageInMonths, String gender) {
    // Simplified weight calculation - replace with WHO standards
    if (gender == 'Laki-laki') {
      return 3.3 + (ageInMonths * 0.5); // Very simplified formula
    } else {
      return 3.2 + (ageInMonths * 0.45); // Very simplified formula
    }
  }
  ...
  Future<void> _exportGrowthData() async {
    try {
      // In a real app, you would implement PDF/Excel export here
      _showSuccessDialog('Fitur export akan segera tersedia!');
    } catch (e) {
      _showErrorDialog('Gagal mengexport data: $e');
    }
  }

  // ============ GROWTH REMINDERS ============

  Future<void> _scheduleGrowthReminder() async {
    try {
      // In a real app, you would schedule local notifications here
      final now = DateTime.now();
      final nextMonth = DateTime(now.year, now.month + 1, now.day);
      
      final reminderData = {
        'child_id': widget.childId,
        'title': 'Pengukuran Pertumbuhan',
        'description': 'Saatnya mengukur berat dan tinggi badan ${widget.childName}',
        'reminder_type': 'growth',
        'reminder_date': nextMonth.toIso8601String().split('T')[0],
        'reminder_time': '09:00',
        'is_repeat': 1,
        'repeat_pattern': 'monthly',
      };
      
      await _database.insertReminder(reminderData);
      _showSuccessDialog('Pengingat bulanan berhasil dijadwalkan!');
    } catch (e) {
      _showErrorDialog('Gagal menjadwalkan pengingat: $e');
    }
  }
  ...
  Widget _buildGrowthRecommendations() {
    ...
    final weight = _latestRecord!['weight']?.toDouble();
    final height = _latestRecord!['height']?.toDouble();
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          Text(
            _getGrowthRecommendation(weight, height),
            style: GoogleFonts.poppins(
              fontSize: 14,
              color: Colors.black87,
              height: 1.5,
  ...
  String _getGrowthRecommendation(double? weight, double? height) {
    // This is a simplified recommendation system
    List<String> recommendations = [];
    
    recommendations.add('• Pastikan ${widget.childName} mendapat nutrisi seimbang setiap hari');
    recommendations.add('• Berikan ASI/susu formula sesuai kebutuhan usia');
    recommendations.add('• Ajak ${widget.childName} beraktivitas fisik yang sesuai usianya');
    recommendations.add('• Jaga pola tidur yang teratur dan cukup');
    recommendations.add('• Lakukan pemeriksaan rutin ke dokter anak');
  ...
  Widget _buildGrowthMilestones() {
    if (_childData == null) return const SizedBox.shrink();

    final birthDate = DateTime.parse(_childData!['date_of_birth']);
        ...
        children: [
          Row(
            children: [
              ...
              const Spacer(),
              TextButton(
                onPressed: _navigateToMilestones,
                child: Text(
                  'Lihat Semua',
                  style: GoogleFonts.poppins(
                    fontSize: 12,
          ...
          const SizedBox(height: 12),
          Text(
            _getGrowthMilestoneText(ageInMonths),
            style: GoogleFonts.poppins(
              fontSize: 14,
              color: Colors.black87,
            ),
  ...
  String _getGrowthMilestoneText(int ageInMonths) {
    if (ageInMonths < 6) {
      return 'Pada usia ini, ${widget.childName} sedang dalam periode pertumbuhan pesat. Pastikan nutrisi dan stimulasi yang optimal.';
    } else if (ageInMonths < 12) {
      return 'Usia 6-12 bulan adalah masa pengenalan MPASI. Pantau pertumbuhan dengan cermat selama transisi ini.';
    } else if (ageInMonths < 24) {
      return 'Toddler phase - pertumbuhan mungkin melambat tetapi aktivitas fisik meningkat. Fokus pada nutrisi seimbang.';
    } else {
      return 'Anak prasekolah membutuhkan nutrisi untuk mendukung aktivitas yang semakin aktif dan perkembangan kognitif.';
    }
  }
}