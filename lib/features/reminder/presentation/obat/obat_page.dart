...
import 'package:primafit/features/reminder/presentation/obat/alarm_service.dart';
import 'package:primafit/features/reminder/presentation/obat/detail_obat.dart';
import 'package:primafit/features/reminder/presentation/obat/update_obat.dart';
...
class _ObatPageState extends State<ObatPage> with SingleTickerProviderStateMixin {
    ...
    return ListTile(
      ...
      leading: RadioGroup<String>(
        ...
        onChanged: (value) {
          ...
          child: Radio<String>(value: field, activeColor: _themeColor),
      ),
      onTap: () {
        setState(() {
      ...
      child: Column(
        ...
        children: [
          ...
          if (!_isSearching)
            ElevatedButton.icon(
              onPressed: () {
                Navigator.pushNamed(context, AppRoutes.medicineInput).then((_) {
                  _refreshObatList();
                });
              },
  ...
  Widget _buildStockIndicator(String stock) {
    ...
    if (stockNum <= 0) {
      ...
      bgColor = _themeColor.withValues(alpha: 0.1);
