...
import 'package:primafit/features/cancer_screening/data/database_usus.dart';
...
class PenyakitUsus {
  ...
  static final Map<String, Map<String, String>> penyakitInfo = {
    'Risiko Tinggi': { //Persentase lebih dari 80%
      'description': 'Anda memiliki faktor risiko yang signifikan untuk kanker usus besar. Diperlukan pemeriksaan lanjutan seperti kolonoskopi dan konsultasi ke spesialis gastroenterologi segera.',
      'treatment': 'Segera lakukan pemeriksaan kolonoskopi, konsultasi dengan dokter spesialis, dan pertimbangkan untuk tes darah samar dalam tinja (FOBT/FIT) secara rutin.',
      'icon': 'exclamationmark_shield_fill',
    },
    'Risiko Sedang': { //Persentase lebih dari 50% dan kurang dari 80%
      'description': 'Anda memiliki beberapa faktor risiko untuk kanker usus besar. Disarankan untuk melakukan pemantauan rutin dan pemeriksaan lebih lanjut.',
      'treatment': 'Lakukan pemeriksaan usus secara rutin, konsultasi dengan dokter untuk penjadwalan kolonoskopi, dan lakukan tes darah samar dalam tinja (FOBT/FIT) setiap tahun.',
      'icon': 'exclamationmark_shield',
    },
    'Risiko Rendah': { //Persentase lebih dari 20% dan kurang dari 50%
      'description': 'Faktor risiko Anda terhadap kanker usus besar terdeteksi minimal. Tetap jaga gaya hidup sehat dan lakukan pemeriksaan rutin.',
      'treatment': 'Pertahankan berat badan ideal, perbanyak konsumsi serat, batasi konsumsi daging merah dan olahan, serta lakukan tes darah samar dalam tinja setiap 1-2 tahun.',
      'icon': 'waveform_path',
    },
    'Tidak Ada Risiko': { //Persentase kurang dari 20%
      'description': 'Tidak ditemukan faktor risiko signifikan kanker usus besar berdasarkan jawaban Anda. Tetap lakukan pemeriksaan rutin untuk pencegahan.',
      'treatment': 'Tetap jaga pola makan seimbang, konsumsi banyak serat dan sayuran, batasi alkohol, pertahankan berat badan ideal, dan rutin berolahraga.',
      'icon': 'checkmark_shield_fill',
    },
  };
}

class OutputPageUsus extends StatefulWidget {
  const OutputPageUsus({super.key});

  @override
  _OutputPageUsusState createState() => _OutputPageUsusState();
}

class _OutputPageUsusState extends State<OutputPageUsus> with SingleTickerProviderStateMixin {
  ...
  Future<File> _generatePDF() async {
        ...
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              pw.Center(
                child: pw.Text(
                  'HASIL DIAGNOSA RISIKO KANKER USUS BESAR',
                  style: pw.TextStyle(
                    fontSize: 18, 
              ...
              pw.SizedBox(height: 20),
              if (_diagnosisResults.isNotEmpty && PenyakitUsus.penyakitInfo.containsKey(_diagnosisResults[0]['penyakit']))
                pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text(
                    ...
                    pw.SizedBox(height: 5),
                    pw.Text(
                      PenyakitUsus.penyakitInfo[_diagnosisResults[0]['penyakit']]!['description'] ?? '',
                    ),
                    ...
                    pw.SizedBox(height: 5),
                    pw.Text(
                      PenyakitUsus.penyakitInfo[_diagnosisResults[0]['penyakit']]!['treatment'] ?? '',
                    ),
                  ],
                ),
              pw.SizedBox(height: 20),
              ...
              pw.SizedBox(height: 5),
              pw.Text(
                'Hasil diagnosa ini hanya sebagai rujukan awal berdasarkan faktor risiko yang diidentifikasi. Konsultasikan dengan dokter atau tenaga medis profesional untuk diagnosis dan pengobatan yang tepat. Deteksi dini kanker usus besar melalui tes seperti kolonoskopi dan tes darah dalam tinja sangat penting untuk pengobatan yang efektif.',
                style: pw.TextStyle(
    ...
    final file = File('${output.path}/hasil_analisis_risiko_kanker_usus_besar.pdf');
  ...
  Future<void> _shareResults() async {
    try {
      ...
      await Share.shareXFiles(
        [XFile(pdfFile.path)],
        text: 'Hasil Analisis Risiko Kanker Usus Besar',
        subject: 'Analisis Risiko Kanker Usus Besar - ${DateFormat('dd MMM yyyy').format(DateTime.now())}',
      );
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _loadDiagnosisResults() async {
    try {
      final results = await DatabaseHelperUsus.instance.getDiagnosisResult(_threshold);
  ...
  Color _getRiskColor(String riskCategory) {
    switch (riskCategory) {
      ...
      case 'Tidak Ada Risiko':
        return Colors.green;
      default:
        return const Color(0xFF64D1DE); // Purple color for colon cancer theme
    }
  }
  ...
  Widget _buildResultCard(Map<String, dynamic> result) {
    ...
    final Map<String, String>? info = PenyakitUsus.penyakitInfo[penyakit];
