...
class _MilestoneParentingPageState extends State<MilestoneParentingPage>
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (_childData != null) {
        DateTime birthDate = DateTime.parse(_childData!['date_of_birth']);
  ...
  Future<void> _createMissingMilestoneEntries() async {
    for (var template in _availableTemplates) {
      bool exists = _childMilestones.any(
  ...
  Map<String, dynamic> get _milestoneStats {
    int total = _childMilestones.length;
    int achieved = _childMilestones.where(
      (milestone) => milestone['is_achieved'] == 1
    ).length;
    
    double percentage = total > 0 ? (achieved / total) * 100 : 0;
    ...
    return Row(
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: _getCategoryColor(milestone['category']).withOpacity(0.1),
            borderRadius: BorderRadius.circular(8),
          ),
          child: Text(
            '${ageInMonths}m',
  ...
  Widget _buildDetailStatus(Map<String, dynamic> milestone, bool isAchieved) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: isAchieved 
              ? const Color(0xFFE9458D).withOpacity(0.3)
