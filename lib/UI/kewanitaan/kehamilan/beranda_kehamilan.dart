...
import 'package:primafit/UI/kewanitaan/kehamilan/tambah_kehamilan.dart';
import 'package:primafit/UI/kewanitaan/kehamilan/tambah_gejala_kehamilan.dart';
import 'package:primafit/UI/kewanitaan/kehamilan/tambah_pemeriksaan.dart';
import 'package:primafit/UI/kewanitaan/kehamilan/tambah_berat_badan.dart';
import 'package:primafit/UI/kewanitaan/kehamilan/tambah_catatan.dart';
import 'package:primafit/UI/kewanitaan/kehamilan/statistik_kehamilan.dart';
import 'package:primafit/UI/kewanitaan/kehamilan/pengaturan_kehamilan.dart';
import 'package:primafit/UI/kewanitaan/kehamilan/info_perkembangan.dart';
import 'package:primafit/UI/kewanitaan/kehamilan/penghitung_tendangan.dart';

class PregnancyTrackerHomePage extends StatefulWidget {
  const PregnancyTrackerHomePage({Key? key}) : super(key: key);
...
class _PregnancyTrackerHomePageState extends State<PregnancyTrackerHomePage> with SingleTickerProviderStateMixin {
  ...
  List<Widget> _getEventsForDay(DateTime day) {
    ...
    // Add checkup events
    if (_checkups.containsKey(dateOnly)) {
      for (var checkup in _checkups[dateOnly]!) {
        events.add(_buildEventChip(
    ...
    // Add symptom events (group by type)
    if (_symptoms.containsKey(dateOnly)) {
      final symptomsByType = <PregnancySymptomType, int>{};
      
      for (var symptom in _symptoms[dateOnly]!) {
        if (symptomsByType.containsKey(symptom.type)) {
          symptomsByType[symptom.type] = symptomsByType[symptom.type]! + 1;
        } else {
          symptomsByType[symptom.type] = 1;
        }
      }
      
      // Show up to 2 symptom types
      int count = 0;
      symptomsByType.forEach((type, number) {
        if (count < 2) {
          events.add(_buildEventChip(
            type.displayName,
      ...
      // If more symptoms, add a +X indicator
      if (symptomsByType.length > 2) {
        events.add(_buildEventChip(
          '+${symptomsByType.length - 2}',
    ...
    // Add weight record
    if (_weights.containsKey(dateOnly)) {
      final weight = _weights[dateOnly]!.first;
      events.add(_buildEventChip(
        '${weight.weight} kg',
    ...
    // Add notes (just show count)
    if (_notes.containsKey(dateOnly)) {
      final noteCount = _notes[dateOnly]!.length;
      if (noteCount > 0) {
        events.add(_buildEventChip(
          '$noteCount catatan',
  ...
  Widget _buildEventChip(String label, Color color, IconData icon) {
    return Container(
        ...
        defaultBuilder: (context, day, focusedDay) {
          ...
          if (_activePregnancy != null && _activePregnancy!.isInPregnancyPeriod(day)) {
            ...
            if (weekNumber > 0) {
              ...
              if (weekNumber <= 13) {
                cellColor = Colors.lightBlue.withOpacity(0.1); // First trimester
              } else if (weekNumber <= 27) {
                cellColor = Colors.teal.withOpacity(0.1); // Second trimester
              } else {
                cellColor = Colors.indigo.withOpacity(0.1); // Third trimester
              }
            }
          }
          ...
          Row(
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: note.type.color.withOpacity(0.2),
