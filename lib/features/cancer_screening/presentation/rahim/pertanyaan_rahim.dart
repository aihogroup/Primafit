...
import 'package:primafit/features/cancer_screening/presentation/rahim/output_rahim.dart';
import 'package:primafit/features/cancer_screening/presentation/rahim/list_pertanyaan.dart';

class KankerRahimPage extends StatefulWidget {
  const KankerRahimPage({super.key});

  @override
  _KankerRahimPageState createState() => _KankerRahimPageState();
}

class _KankerRahimPageState extends State<KankerRahimPage> with SingleTickerProviderStateMixin {
  ...
  void _nextPage() {
    if (_currentPage < KankerRahimQuestions.activeQuestions(_answers).length - 1) {
      _animateToNextPage();
    } else {
      _submitAnswers();
    }
  ...
  Future<void> _submitAnswers() async {
    ...
    try {
      ...
      for (var question in KankerRahimQuestions.questions) {
        final String id = question['id'];
        
        // Check if this question should be skipped
        bool shouldSkip = false;
        for (var otherQuestion in KankerRahimQuestions.questions) {
      ...
      await DatabaseHelperRahim.instance.saveUserAnswers(userAnswers);
      ...
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(builder: (context) => const OutputPageRahim()),
      );
    } catch (e) {
      if (!mounted) return;
          ...
          onTap: () {
            setState(() {
              ...
              if (answer == 'Ya') {
                ...
                final currentQuestion = KankerRahimQuestions.questions.firstWhere(
                  (q) => q['id'] == questionId,
                  orElse: () => {},
                );
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Text(
                      'Pertanyaan ${index + 1}/${KankerRahimQuestions.activeQuestions(_answers).length}',
                      style: GoogleFonts.poppins(
                        fontSize: 14,
                        fontWeight: FontWeight.w500,
                        color: Colors.grey.shade700,
                      ...
                      child: Stack(
                        children: [
                          AnimatedFractionallySizedBox(
                            duration: const Duration(milliseconds: 300),
                            widthFactor: (index + 1) / KankerRahimQuestions.activeQuestions(_answers).length,
                            child: Container(
                              decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(10),
                ...
                Row(
                  ...
                  children: [
                    ...
                    if (index < KankerRahimQuestions.activeQuestions(_answers).length - 1)
                      ElevatedButton.icon(
  ...
  Widget build(BuildContext context) {
    ...
    final activeQuestions = KankerRahimQuestions.activeQuestions(_answers);
