...
import 'package:primafit/database/skincare/database_facialwash.dart';
...
class FacialWashCategories {
  ...
  static final Map<String, Map<String, String>> categoryInfo = {
    'untuk_kulit_berminyak': {
      'title': 'Facial Wash untuk Kulit Berminyak',
      'description': 'Facial wash ini diformulasikan khusus untuk kulit berminyak, membantu mengontrol produksi sebum berlebih dan mencegah pori tersumbat.',
      'recommendation': 'Gunakan facial wash ini 2 kali sehari, pagi dan malam hari. Pijat lembut pada wajah basah dan bilas dengan air bersih.',
      'icon': 'drop_fill',
    },
    'untuk_kulit_kering': {
      'title': 'Facial Wash untuk Kulit Kering',
      'description': 'Formula lembut yang membersihkan tanpa menghilangkan kelembaban alami kulit, dengan kandungan pelembab untuk mengatasi kulit kering.',
      'recommendation': 'Gunakan 1-2 kali sehari. Hindari air terlalu panas, dan segera aplikasikan pelembab setelah mencuci wajah.',
      'icon': 'waveform_path',
    },
    'untuk_kulit_sensitif': {
      'title': 'Facial Wash untuk Kulit Sensitif',
      'description': 'Diformulasikan tanpa bahan iritan dan pewangi, lembut untuk kulit sensitif namun tetap efektif membersihkan.',
      'recommendation': 'Gunakan dengan lembut 1-2 kali sehari. Jangan menggosok kuat, tepuk-tepuk hingga kering dan aplikasikan pelembab untuk kulit sensitif.',
      'icon': 'exclamationmark_shield',
    },
    'untuk_anti_jerawat': {
      'title': 'Facial Wash Anti Jerawat',
      'description': 'Mengandung bahan aktif seperti Salicylic Acid, Tea Tree Oil, atau Benzoyl Peroxide untuk melawan bakteri penyebab jerawat.',
      'recommendation': 'Gunakan 2 kali sehari. Mulai dengan frekuensi lebih rendah jika terjadi iritasi. Kombinasikan dengan pelembab non-comedogenic.',
      'icon': 'bandage',
    },
    'untuk_pencerah': {
      'title': 'Facial Wash Pencerah',
      'description': 'Membantu mencerahkan kulit kusam dan menyamarkan noda hitam dengan kandungan Vitamin C, Arbutin, atau AHA.',
      'recommendation': 'Gunakan 2 kali sehari. Selalu gunakan tabir surya minimal SPF 30 saat siang hari, terutama saat menggunakan produk pencerah.',
      'icon': 'sun_max',
    },
    'untuk_anti_aging': {
      'title': 'Facial Wash Anti Aging',
      'description': 'Diformulasikan dengan antioksidan dan peptida yang membantu melawan tanda-tanda penuaan dan meningkatkan elastisitas kulit.',
      'recommendation': 'Gunakan 2 kali sehari. Lengkapi dengan serum anti-aging dan pelembab malam yang mengandung retinol untuk hasil optimal.',
      'icon': 'arrow_2_circlepath',
    },
    'untuk_pori_besar': {
      'title': 'Facial Wash untuk Pori Besar',
      'description': 'Membantu mengecilkan tampilan pori-pori dengan membersihkan secara mendalam dan mengatur produksi sebum.',
      'recommendation': 'Gunakan 2 kali sehari. Kombinasikan dengan exfoliator berbahan BHA 1-2 kali seminggu untuk hasil lebih baik.',
      'icon': 'circle_grid_3x3',
    },
    'untuk_pembersih_mendalam': {
      'title': 'Facial Wash Pembersih Mendalam',
      'description': 'Efektif membersihkan makeup, sunscreen, dan kotoran tanpa meninggalkan residu pada wajah.',
      'recommendation': 'Ideal untuk double cleansing, gunakan setelah cleansing oil/micellar water untuk membersihkan sisa makeup atau sunscreen.',
      'icon': 'wand_stars',
    },
  };
}

class OutputPageFacial extends StatefulWidget {
  const OutputPageFacial({Key? key}) : super(key: key);

  @override
  _OutputPageFacialState createState() => _OutputPageFacialState();
}

class _OutputPageFacialState extends State<OutputPageFacial> with SingleTickerProviderStateMixin {
  ...
  Future<File> _generatePDF() async {
              ...
              if (_recommendationResult != null && _recommendationResult!['recommendation']['top_category'] != null)
                pw.Column(
                  ...
                  children: [
                    ...
                    pw.SizedBox(height: 5),
                    pw.Text(
                      FacialWashCategories.categoryInfo[_recommendationResult!['recommendation']['top_category']]?['description'] ?? '',
                    ),
                    pw.SizedBox(height: 10),
                    pw.Text(
                    ...
                    pw.SizedBox(height: 5),
                    pw.Text(
                      FacialWashCategories.categoryInfo[_recommendationResult!['recommendation']['top_category']]?['recommendation'] ?? '',
                    ),
                  ],
                ),
    ...
    final file = File('${output.path}/rekomendasi_facial_wash.pdf');
  ...
  Future<void> _shareResults() async {
    try {
      ...
      await Share.shareXFiles(
        [XFile(pdfFile.path)],
        text: 'Rekomendasi Facial Wash Untukmu',
        subject: 'Rekomendasi Facial Wash - ${DateFormat('dd MMM yyyy').format(DateTime.now())}',
      );
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _loadRecommendationResults() async {
    try {
      final result = await DatabaseHelperFacial.instance.getLatestRecommendation();
  ...
  Widget _buildRecommendationCard() {
    ...
    final Map<String, String>? info = FacialWashCategories.categoryInfo[category];
