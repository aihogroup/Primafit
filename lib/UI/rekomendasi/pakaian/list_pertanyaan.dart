...
class FashionQuestions {
  ...
  static List<Map<String, dynamic>> activeQuestions(Map<String, String> answers) {
    return questions.where((q) {
      ...
      // Skip pertanyaan body type yang spesifik gender jika tidak sesuai
      if (questionId == 'body_type' && answers.containsKey('gender')) {
        String gender = answers['gender'] ?? '';
        List<String> options = List<String>.from(q['options']);
        
        // Filter opsi berdasarkan gender
        if (gender == 'Pria') {
          q['options'] = options.where((option) => option.startsWith('Pria') || option == 'Prefer Not to Specify').toList();
        } else if (gender == 'Wanita') {
          q['options'] = options.where((option) => option.startsWith('Wanita') || option == 'Prefer Not to Specify').toList();
        }
        // Untuk non-binary, tetap tampilkan semua opsi
      }
      
      // Jika style_icons opsional dan user tidak menjawab pertanyaan tersebut, bisa dilewati
      if (questionId == 'style_icons' && answers.containsKey('event_type') && 
          answers.containsKey('style_preference') && 
          answers.containsKey('color_preference')) {
        // Buat style_icons menjadi opsional jika user sudah menjawab pertanyaan penting lainnya
        // shouldSkip tetap false, hanya set di UI bisa skip ke pertanyaan berikutnya
      }
  ...
  static Map<String, dynamic> calculateFashionRecommendation(Map<String, dynamic> answers) {
    ...
    try {
      ...
      String gender = answers['gender'] ?? 'Non-binary/Gender Neutral';
      String eventType = answers['event_type'] ?? 'Casual (Sehari-hari/Weekend)';
      String stylePreference = answers['style_preference'] ?? 'Casual/Relaxed';
      String colorPreference = answers['color_preference'] ?? 'Warna Netral (Hitam, Putih, Abu-abu, Beige)';
      String bodyType = answers['body_type'] ?? 'Prefer Not to Specify';
      String season = answers['season'] ?? 'Cuaca Bervariasi/Transisi';
