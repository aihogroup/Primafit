...
import 'package:primafit/UI/pengingat/jadwal/notification_service.dart';
...
class DatabaseAgenda {
  ...
  Future<void> _scheduleNextRepeatingAgenda(Agenda agenda) async {
    ...
    String nextDate = "${date.year}-${date.month.toString().padLeft(2, '0')}-${date.day.toString().padLeft(2, '0')}";
    
    // Create next agenda
    Agenda nextAgenda = agenda.copyWithNewDate(nextDate);
