...
import 'package:primafit/database/catat/database_bmi.dart';

class UpdateBmiScreen extends StatefulWidget {
  final int BmiId;
  
  const UpdateBmiScreen({
    Key? key,
    required this.BmiId,
  }) : super(key: key);

  @override
  State<UpdateBmiScreen> createState() => _UpdateBmiScreenState();
}

class _UpdateBmiScreenState extends State<UpdateBmiScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadBmiData() async {
    try {
      final data = await IndeksMassaTubuhDatabaseHelper.instance.getIndeksMassaTubuhById(widget.BmiId);
      if (data != null) {
        setState(() {
          _currentData = data;
          _beratController.text = data.berat;
          _tinggiController.text = data.tinggi;
  ...
  void _updateData() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        final updatedBmi = IndeksMassaTubuh(
          id: widget.BmiId,
          tanggal: formattedDate,
          waktu: formattedTime,
          berat: _beratController.text,
          tinggi: _tinggiController.text,
          catatan: _catatanController.text, bmi: '',
        );
        
        await IndeksMassaTubuhDatabaseHelper.instance.updateIndeksMassaTubuh(updatedBmi);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          'Edit Catatan Indeks Massa Tubuh',
                  ...
                  child: Padding(
                    ...
                    child: Form(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            'Perbarui data pemeriksaan Bmi Anda',
                            style: GoogleFonts.poppins(
                              fontSize: 14,
